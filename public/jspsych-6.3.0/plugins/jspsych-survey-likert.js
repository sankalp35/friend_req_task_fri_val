...
jsPsych.plugins['survey-likert'] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      view_duration: {
        ...
        default: 0,
        description: 'How long before next is clickable.'
      },
      qn_sequence_order: {
        type: jsPsych.plugins.parameterType.INT,
        array: true,
        pretty_name: 'Sequence order',
        default: null,
        description: 'Define a 2 item array. First is the order of the present questionnaire in the current set, and second is the total number of questionnaires in the current set'
      }
    }
  }
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // show preamble text
    if(trial.qn_sequence_order !== null){
      html += '<div id="qn_seq_order">This questionnaire is '+ trial.qn_sequence_order[0] + ' out of ' + trial.qn_sequence_order[1]+'</div>'; 
    }
  ...
  //  setTimeout(function(){ document.getElementById('jspsych-survey-likert-next').disabled=false}, trial.view_duration); //this is added
    ...
    display_element.querySelector('#jspsych-survey-likert-form').addEventListener('submit', function(e){
      ...
      var fast_count=0;
      if(response_time <= trial.view_duration) {
        fast_count++;
        var play = confirm("You seem to have completed these questions too fast. Please press 'Cancel' to reconsider your responses or press 'OK' to continue.");
        if (play==false) {
          return;
        }
      }
